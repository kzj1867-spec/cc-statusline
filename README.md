...
[![npm version](https://badge.fury.io/js/@zach19/cc-statusline.svg)](https://badge.fury.io/js/@zach19/cc-statusline)
...
> 💡 **兼容说明**：旧命令 `npx @zach19/cc-statusline@latest` 仍会继续工作并自动调用新包，但将显示迁移提示。建议尽快将现有配置更新为 `npx @zach19/cc-statusline@latest` 以保持一致。
...
```bash
# 显示所有组件（推荐）
npx @zach19/cc-statusline@latest --preset PMBTUS --theme powerline

# 只显示模型、Token和使用量
npx @zach19/cc-statusline@latest --preset MTU --theme classic

# 只显示分支和Token信息
npx @zach19/cc-statusline@latest --preset BT --theme capsule
```
...
执行 `npx @zach19/cc-statusline@latest config init -w` 可以一次性生成主配置并复制所有小组件模板。
...
npx @zach19/cc-statusline@latest --force-nerd-font

# 强制启用Emoji图标（需要终端确实支持Emoji，如MacOS，否则会显示乱码）
npx @zach19/cc-statusline@latest --force-emoji  

# 强制启用纯文本模式
npx @zach19/cc-statusline@latest --force-text
```
...
- 适用: 特定项目的个性化配置
- 初始化: `npx @zach19/cc-statusline@latest config init`
- 初始化并复制组件模板: `npx @zach19/cc-statusline@latest config init -w`

**用户级配置** (优先级: 低)

- 路径: `~/.claude/statusline-pro/config.toml`
- 适用: 全局默认配置，适用于所有项目
- 初始化: `npx @zach19/cc-statusline@latest config init -g`
- 初始化并复制组件模板: `npx @zach19/cc-statusline@latest config init -w -g`
...
```bash
# 初始化项目级配置（推荐）
npx @zach19/cc-statusline@latest config init

# 初始化项目级配置并复制组件模板
npx @zach19/cc-statusline@latest config init -w

# 初始化全局配置
npx @zach19/cc-statusline@latest config init -g

# 强制重新初始化（覆盖现有配置）
npx @zach19/cc-statusline@latest config init --force
```
...
```bash
# 错误示例：status line command failed: npx @zach19/cc-statusline@latest
...
1. 确认使用最新版本：`npx @zach19/cc-statusline@latest --version`
2. 若仍提示旧二进制，可清理 npm 缓存后重试：
   ```bash
   npm cache clean --force
   npx @zach19/cc-statusline@latest --version
   ```
...
```bash
# 检查终端颜色支持，可以禁用颜色
npx @zach19/cc-statusline@latest --no-color
```
...
```bash
# 测试基本功能
echo '{"model":{"id":"claude-sonnet-4"}}' | npx @zach19/cc-statusline@latest

# 测试特定预设和主题
echo '{"model":{"id":"claude-sonnet-4"}}' | npx @zach19/cc-statusline@latest --preset MT --theme classic
```
...
> 💡 **Compatibility Note**: The legacy command `npx @zach19/cc-statusline@latest` still works and forwards to the new package, but will emit a migration warning. Updating existing configs to `npx @zach19/cc-statusline@latest` keeps future upgrades smooth.
...
Run `npx @zach19/cc-statusline@latest config init -w` to generate the main config and copy all widget templates in one step.
...
- Initialization: `npx @zach19/cc-statusline@latest config init`
- Initialization with component templates: `npx @zach19/cc-statusline@latest config init -w`
...
- Initialization: `npx @zach19/cc-statusline@latest config init -g`
- Initialization with component templates: `npx @zach19/cc-statusline@latest config init -w -g`
...
```bash
# Error example: status line command failed: npx @zach19/cc-statusline@latest
